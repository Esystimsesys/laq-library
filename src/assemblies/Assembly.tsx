...
function LoadAssembly({ entry }: { entry: AssemblyEntry }) {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.BASE_URL}${entry.guidePath}?v=${entry.revision}`, { signal: controller.signal })
...
function Journey({ entry, guide }: { entry: AssemblyEntry; guide: Guide }) {
  ...
  function go(index: number) {
    ...
    persist({ at: next === 0 ? progress.at : next, checked: progress.checked })
  ...
  return <div className={styles.page}>
    ...
    {mapOpen && <section className={styles.map} aria-label="ぜんたいの ながれ">
      ...
      <ol className={styles.instructionMap}>{tasks.map((item, i) => {
        ...
        return <li key={item.key}><button onClick={() => go(steps.indexOf(item))} aria-current={item.key === current.key ? 'step' : undefined} aria-label={`てじゅん ${i + 1} ${item.title}`}>
          ...
          <span><strong>{item.title}</strong><small>{item.source?.inputs ? `${item.source.inputs.map(label).join(' ＋ ')} → ${label(id)}` : label(id)}</small></span>
        </button></li>
      })}</ol>
    ...
    <nav className={styles.bottom} aria-label="てじゅんを すすめる">
      ...
      {current.phase !== 'done' && <button className={styles.primary} disabled={renderedKey !== current.key} onClick={() => go(at + 1)}>{current.phase === 'welcome' ? 'つくりはじめる →' : 'つぎへ →'}</button>}
    </nav>
  </div>
}
