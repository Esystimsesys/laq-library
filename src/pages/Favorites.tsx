...
export default function Favorites() {
  ...
  const shown = list.filter(m => status === 'all' || (status === 'made' ? Boolean(state.made[m.id]) : !state.made[m.id]))
  ...
  return <div className={page.page}>
    ...
    <ModelGrid models={shown} favoriteButton={false} paged={false} empty={list.length === 0 ?
      ...
      <EmptyState
        title={status === 'notMade' ? 'ここにある さくひんは ぜんぶ つくったね！' : 'つくった きろくは まだ ありません'}
        hint={status === 'notMade' ? 'あたらしい さくひんを さがしてみよう。' : 'つくれたら、さくひんの ページで「つくった！」を おしてね。'}
