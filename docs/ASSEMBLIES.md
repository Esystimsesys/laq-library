...
メタモン・ジャローダ・デスカーン・イワパレス・ネギガナイト・オオスズメバチの6作品を登録している。
...
オオスズメバチは `original:giant-hornet` → `giant-hornet` revision 1。68ピース・8まとまり・6表示手順。
ミニシャフト／ミニホイールを含み、ユーザーが形状・接続・表示手順を確認済み。実物再組立は未確認。
[制作記録](GIANT-HORNET-3D.md)と[振り返り](GIANT-HORNET-RETROSPECTIVE.md)を参照。
...
レビュー済み出力の取り込みには `--use-source-reading` を付け、確認したパーツ名・順序を保持する。
...
接続操作と `assembly` は再生成しない。公開ファイルは初回読み込みを軽くするため1行JSONとし、
制作・検証だけで使う根拠、確度、検証集計、モデル説明、接続操作の説明文を除く。
座標の小数は表示精度に影響しない6桁へ丸めるが、私有の入力ガイドは変更しない。表示用の設定で指定された塊だけ、
...
`--title` の省略時は作品名、`--article` の省略時は登録済み作品の `sourceUrl` を使う。
外部ページを持たないオリジナル作品では公開ガイドの `article` は空文字にする。
初回の `revision` は 1。再取り込み時は既存値を保ち、
...
制作室では `reading.stepGroups: [{ id, title, keys }]` で、連続する手順の表示だけを1画面へまとめられる。
元の `sequence`、unitsの所属、接続操作を維持し、図のラベルと分解表示も元のまとまりを使用する。
keysは既存の手順キーを順に2件以上指定し、グループ間の重複は禁止。コメントの添付先は `group:<id>`。
各グループに `explodeGroups`（パーツID配列の配列）を指定すると、unitsをまたぐ分解時のまとまりも定義できる。統合手順の新規パーツを重複・欠落なく1回ずつ含める。
公開画面も同じ `stepGroups` を読み、制作室で確認した表示手順だけを一覧と進行に使う。
...
- 塊内の分解表示は中心からの変位を距離に比例して広げ、同じ方向のパーツも離す。スライダー0では元の配置を保持する。
  塊の2段目以降（`presentation: "join"`）は、前の段までに作った部分を動かさず、新しく付けるパーツをつながりごとに
  ひとまとまりにして、合体手順と同じく接続の向きへ離す。作成済みの塊はばらさない。
  複数の旧手順を1枚へ統合した場合は `explodeGroups` に新規パーツをまとまり別で指定し、分解時も各まとまりを崩さない。
  各新規パーツを重複なく、いずれか1つのグループへ含める。
- 分解スライダーは形状を再生成せず、手順開始時に求めた移動量・境界を使って既存パーツを動かす。連続入力はrequestAnimationFrameでまとめ、操作終了時には最後の値を反映する。キャンバスのサイズ変更は実寸が変わったときだけ行う。
- 不透明パーツの表面と縁取りをパーツごとにまとめて描画する。元の頂点・法線・線分は保持し、選択・独立移動に対応。透明パーツは描画順を保つため統合しない。
- No.3/4の表裏は一続きの形状で生成し、内部の左右の継ぎ目を輪郭線にしない。No.6/7も角柱と接続部分を一体の外表面として生成する。外周・溝・端面のくぼみは維持し、基本形状をキャッシュして繰り返し計算を避ける。
- [パーツの横並び確認](../tools/assembly-review/parts.html)をローカルのブラウザで開くと、No.3/6/7を同じ色・倍率・向きで比較できる。正面・端面・裏側のボタンとドラッグ回転に対応。No.7の細部が未確認という位置づけは変わらない。
...
共通ビューアーはPWAの事前キャッシュに含める。作品別の3DガイドJSONは事前取得せず、
開いた作品だけを実行時キャッシュへ保存してオフライン再訪を可能にする。
描画HTMLはSPAの代替ページへ変換しない。
